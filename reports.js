// ============ Reports Dashboard (Firestore multi-clinic) ============
...
async function loadFromFirestore() {
  ...
  currentUserEmail = (profile?.email || profile?.user?.email || '').toLowerCase();
  if (!clinicId) {
    console.warn('No clinicId available; reports cannot load.');
    bookings = [];
    return;
  }
  try {
    const snap = await getDocs(collection(db, 'clinics', clinicId, 'bookings'));
    bookings = snap.docs.map(d => ({ id: d.id, ...d.data() }));
  } catch (err) {
    console.error('Failed to load bookings:', err);
    bookings = [];
  }
  // Audit log is still local-only (kept for backward compatibility)
  try { auditLog = JSON.parse(localStorage.getItem('clinic_audit_v1') || '[]'); }
  catch { auditLog = []; }
}
...
function renderDoctor(rows) {
  ...
  const body = document.querySelector('#doctorTable tbody');
  if (body) body.innerHTML = labels.map(d =>
...
function renderRevenue(rows) {
  ...
  if (body) body.innerHTML = Object.entries(tbl).sort().map(([k,v]) => {
    const [d,doc] = k.split('||');
...
function renderPayment(rows) {
  ...
  const body = document.querySelector('#paymentTable tbody');
  if (body) body.innerHTML = labels.map(l =>
...
function renderPatient(rows) {
  ...
  if (body) body.innerHTML = list.map(p =>
...
function renderNoShow(rows) {
  ...
  const body = document.querySelector('#noshowTable tbody');
  if (body) body.innerHTML = ns.map(b =>
...
function renderContact(rows) {
  ...
  const body = document.querySelector('#contactTable tbody');
  if (body) body.innerHTML = Object.values(map).map(p =>
...
function renderAudit() {
  const rows = [...auditLog].filter(a => (a.user || '').toLowerCase() === currentUserEmail).reverse();
...
document.addEventListener('click', e => {
  ...
  if (t === 'schedule') {
    ...
    exportSheet(auditLog.filter(a => (a.user || '').toLowerCase() === currentUserEmail).map(a=>{const i=bookingInfo(a.bookingId, a.details);return{'الإجراء':a.action,'المستخدم':a.user,'التاريخ':a.timestamp,'اسم العميل':i.name,'وسيلة الاتصال':i.contact,'التفاصيل':a.details||''};}), 'audit');
...
// ---------- Bootstrap (wait for auth, then load + render) ----------
(async function init() {
...
// Refresh when window regains focus (in case bookings changed on another tab)
