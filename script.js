// ============ State ============
// NOTE: bookings now live in Firestore at clinics/{clinicId}/bookings.
// The in-memory `bookings` array drives all UI rendering exactly as before.
...
let bookings = [];                 // hydrated from Firestore on auth:ready
let auditLog = JSON.parse(localStorage.getItem(AUDIT_KEY) || '[]');
let currentUser = 'anonymous';     // overwritten once auth is ready
let clinicId = null;               // resolved from users/{uid}.clinicId

function bookingsCol() {
  if (!clinicId) throw new Error('clinicId not loaded yet');
  return collection(db, 'clinics', clinicId, 'bookings');
}
async function fsSaveBooking(b) {
  await setDoc(doc(bookingsCol(), b.id), b);
}
async function fsDeleteBooking(id) {
  await deleteDoc(doc(bookingsCol(), id));
}
async function fsUpdateBooking(id, patch) {
  await updateDoc(doc(bookingsCol(), id), patch);
}
async function fsLoadBookings() {
...
// Per-account storage: each logged-in user has their own doctors/specialties list
function _userScope() {
  const p = window.currentUserProfile;
  return p ? (p.email || p.uid || 'user') : '__pending__';
}
function DOCTORS_KEY() { return 'clinic_doctors_v1::' + _userScope(); }
let doctors = JSON.parse(localStorage.getItem(DOCTORS_KEY()) || '[]');

// Free-typed entries remembered per account
function FREE_KEY() { return 'clinic_free_entries_v1::' + _userScope(); }
function loadFree() { return JSON.parse(localStorage.getItem(FREE_KEY()) || '{"specialties":[],"doctors":[]}'); }
function saveFree(d) { localStorage.setItem(FREE_KEY(), JSON.stringify(d)); }
function rememberEntry(kind, val) {
  val = (val || '').trim();
  if (!val) return;
  const d = loadFree();
  if (!d[kind].includes(val)) { d[kind].push(val); saveFree(d); }
}

function uniqueSpecialties() {
  const fromDoctors = doctors.map(d => d.specialty).filter(Boolean);
  const free = loadFree().specialties;
  return [...new Set([...fromDoctors, ...free])];
}

function populateSpecialtyDropdown() {
  const dl = $('#specialtyList');
  if (!dl) return;
  dl.innerHTML = uniqueSpecialties().map(s => `<option value="${escape(s)}"></option>`).join('');
}
...
function populateDoctorDropdown() {
  ...
  const fromDoctors = doctors.filter(d => !spec || d.specialty === spec).map(d => d.name);
  const free = loadFree().doctors;
  const list = [...new Set([...fromDoctors, ...free])];
  dl.innerHTML = list.map(n => `<option value="${escape(n)}"></option>`).join('');
}
...
saveBtn.addEventListener('click', async () => {
  ...
  if (!clinicId) { toast('لم يتم تحميل بيانات العيادة'); return; }
  const data = readForm();
  rememberEntry('specialties', data.specialty);
  rememberEntry('doctors', data.doctor);
  try {
    await fsSaveBooking(data);
    refreshDoctorsUI();
    if (editingId) {
...
async function updateStatus(id, value) {
  ...
  b.status = value;
  try {
    await fsUpdateBooking(id, { status: value });
...
$('#importExcelFile').addEventListener('change', async (e) => {
  ...
  try {
    ...
    if (clinicId) {
      await Promise.all(toPersist.map(b => fsSaveBooking(b).catch(err => console.error(err))));
    }
    renderTable();
...
// ============ Firebase init: wait for auth, then load bookings ============
(async () => {
  try {
    const profile = await (window.authReady || Promise.resolve(null));
    if (!profile) return; // auth-guard already redirected
    currentUser = profile.email || profile.uid || 'user';
    clinicId = profile.clinicId;
    // Reload per-account doctors/specialties now that we know who's logged in
    doctors = JSON.parse(localStorage.getItem(DOCTORS_KEY()) || '[]');
    refreshDoctorsUI();
    if (!clinicId) {
      toast('لم يتم ربط حسابك بعيادة. تواصل مع المدير.');
      return;
    }
    await fsLoadBookings();
    renderTable();
    renderTimeSlots();
  } catch (err) {
    console.error('Firebase init failed:', err);
    toast('تعذر تحميل الحجوزات');
  }
})();
