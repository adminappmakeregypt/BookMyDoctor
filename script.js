...
function populateDoctorDropdown() {
  ...
  dl.innerHTML = list.map(d => `<option value="${escape(d.name)}">${d.specialty ? escape(d.specialty) : ''}</option>`).join('');
}
...
$('#specialty').addEventListener('input', populateDoctorDropdown);

// Persist a newly-typed specialty/doctor so it appears in next visit's dropdown
function rememberDoctorEntry(specialty, doctorName) {
  ...
  if (doctorName) {
    ...
    if (!existing) {
      doctors.push({ name: doctorName, specialty: specialty || '' });
      changed = true;
    } else if (specialty && !existing.specialty) {
      existing.specialty = specialty;
      changed = true;
    }
  } else if (specialty && !uniqueSpecialties().includes(specialty)) {
    // specialty only (no doctor yet) — store as a placeholder entry so it appears in suggestions
    doctors.push({ name: '', specialty });
    changed = true;
  }
  if (changed) {
    try { localStorage.setItem(DOCTORS_KEY, JSON.stringify(doctors)); } catch(e){}
    refreshDoctorsUI();
  }
}
...
saveBtn.addEventListener('click', async () => {
  ...
  try {
    ...
    rememberDoctorEntry(data.specialty, data.doctor);
    if (editingId) {
